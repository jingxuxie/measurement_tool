...
class AMI430(IPInstrument):
    ...
    def enter_persistent_mode(self) -> bool:
        
        if self.switch_heater.in_persistent_mode():
            return True
        if (self.field() < 0.18) and (self.field() > -0.18):
            return True
        else:
            self.pause()
            self.write("W_KEY_ESC")
            self._sleep(1)
            self.write("W_KEY_PSWITCH")
            self._sleep(1)
            self.write("W_KEY_ENTER")
            self._sleep(1)
            ...
            self._sleep(1)
            i = 0;
            while (not self.switch_heater.in_persistent_mode()) and (i < 3600):
                self._sleep(self.ramping_state_check_interval())
                i += 1
        return self.switch_heater.in_persistent_mode()
        
    def exit_persistent_mode(self) -> bool:
    
        if not self.switch_heater.in_persistent_mode():
            return True
        self.write("W_KEY_PSWITCH")
        self._sleep(1)
        self.write("W_KEY_ENTER")
        i = 0
        self._sleep(0.5)
        while (self.ramping_state() != "heating switch") and (i < 600):
            self._sleep(self.ramping_state_check_interval())
            i += 1
        self._sleep(1)
        ...
        self.write("W_KEY_ENTER")
        self._sleep(1)
        return not self.switch_heater.in_persistent_mode()
    ...
    def check_quenched_interval(self) -> float:
