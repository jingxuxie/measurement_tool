...
class SR830(VisaInstrument):
    ...
    def __init__(self, name, address, autorange: bool = True, **kwargs):
    ...
    def rescale_VX(self):
        ...
        x = self.X()
        n = self._VOLT_TO_N[sens]
        dn = 0
        
        #10nA * 10 maximum 100kohm minimum 10ohm
        sens_min = 11 
        sens_max = 20
        while sens_min <= n + dn <= sens_max:  
            if abs(x) > sens * 0.9:
                dn = 1
            elif abs(x) < sens * 0.1:
                dn = -1
            else:
                sleep(10 * self.time_constant())
                return self.X()
            n += dn
            if n < sens_min:
                n = sens_min
                ...
                n = sens_max
            sens = self._N_TO_VOLT[n]
            self.sensitivity.set(sens)
            sleep(5 * self.time_constant())
            x = self.X()
        return x
