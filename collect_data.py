...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        cmap_list = ['seismic', 'seismic_r', 'RdBu', 'RdBu_r', 'autumn', 'bwr', 'cool', 'coolwarm', 'hot', 'jet', 'rainbow', 'viridis', 'plasma', 'winter']
    ...
    def initialize_instruments(self):
        ...
        try:
            for i in range(self.read_num):
                ...
                elif self.read_info[i]['MeterType'] == 'SR830 X' or self.read_info[i]['MeterType'] == 'SR830 Y' or self.read_info[i]['MeterType'] == 'SR830 Theta' or self.read_info[i]['MeterType'] == 'SR830 R':
            ...
            for i in range(self.loop_num): 
                if self.loop_info[i]['MeterType'][:5] == 'K2502': #check is K2502 error
                    if self.value_list[i][0] != -self.loop_params[i]['StartPoint'] or self.value_list[i][-1] != -self.loop_params[i]['EndPoint']:
                        ...
                        return
                    if -90 < self.loop_params[i]['StartPoint'] < 90 and -90 < self.loop_params[i]['EndPoint'] < 90:
                    ...
                    if -16.01 < self.loop_params[i]['StartPoint'] < 4.01 and -16.01 < self.loop_params[i]['EndPoint'] < 4.01:
